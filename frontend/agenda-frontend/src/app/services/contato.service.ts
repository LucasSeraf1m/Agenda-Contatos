...
export class ContatoService {
  ...
  private contatoSignal = signal<{content: Contato[]; totalElements: number }>({ content: [], totalElements: 0 });

  constructor(private http: HttpClient) { }

  listarContatos(){
    this.http.get<{ content: Contato[]; totalElements: number }>('http://localhost:8080/agenda/contatos')
  ...
  buscarContato(idContato: number) {
    return this.contatoSignal().content.find(contato => contato.idContato == idContato);
  }
}
